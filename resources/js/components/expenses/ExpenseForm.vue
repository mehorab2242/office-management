...
<template>
    <form class="space-y-6" @submit.prevent="submit">
        <div class="grid gap-5 sm:grid-cols-2">
            <div><label class="label" for="expense-date">Expense date *</label><DatePicker id="expense-date" v-model="form.expense_date" :aria-invalid="!!errorFor('expense_date')" aria-label="Expense date" /><p v-if="errorFor('expense_date')" class="error-text">{{ errorFor('expense_date') }}</p></div>
