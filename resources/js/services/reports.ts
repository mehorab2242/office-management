...
export async function monthlyReport(year: number, month: number, filters: Record<string, unknown> = {}): Promise<MonthlyReport> { const { data } = await api.get<ApiResponse<MonthlyReport>>('/reports/monthly', { params: { year, month, ...filters } }); return data.data; }
