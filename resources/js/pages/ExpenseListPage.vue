...
const filters = reactive({
    search: '', category_id: '', payment_status: '', payment_method: '', date_from: '', date_to: '',
...
function clearFilters(): void {
    Object.assign(filters, { search: '', category_id: '', payment_status: '', payment_method: '', date_from: '', date_to: '', amount_min: '', amount_max: '', sort: 'expense_date', direction: 'desc' });
    load();
}
...
function canEditExpense(expense: Expense): boolean { return auth.isAdmin || expense.created_by?.id === auth.user?.id; }
...
watch([
    () => filters.category_id, () => filters.payment_status, () => filters.payment_method,
    ...
    <AppLayout>
        ...
        <section class="card mt-6">
            <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-8">
                ...
                <select v-model="filters.payment_status" class="field mt-0" aria-label="Payment status"><option value="">All statuses</option><option value="paid">Paid</option><option value="pending">Pending</option><option value="unpaid">Unpaid</option><option value="unspecified">Unspecified</option></select>
                <input v-model="filters.payment_method" class="field mt-0" aria-label="Payment method" placeholder="Payment method" />
                <input v-model="filters.date_from" type="date" class="field mt-0" aria-label="From date" />
                <input v-model="filters.date_to" type="date" class="field mt-0" aria-label="To date" />
        ...
        <section v-else class="mt-6 overflow-hidden rounded-xl border bg-white">
            ...
            <div v-else>
                <div class="hidden overflow-x-auto md:block"><table class="w-full text-left text-sm"><thead class="border-b bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th class="px-5 py-3 font-medium">Date</th><th class="px-5 py-3 font-medium">Description</th><th class="px-5 py-3 font-medium">Category</th><th class="px-5 py-3 font-medium">Status</th><th class="px-5 py-3 font-medium">Payment method</th><th class="px-5 py-3 font-medium">Created by</th><th class="px-5 py-3 text-right font-medium">Amount</th><th class="px-5 py-3 text-right font-medium">Actions</th></tr></thead><tbody class="divide-y"><tr v-for="expense in expenses" :key="expense.id" class="hover:bg-slate-50"><td class="whitespace-nowrap px-5 py-4">{{ formatDate(expense.expense_date) }}</td><td class="max-w-xs px-5 py-4"><button class="truncate text-left font-medium hover:text-brand-700" @click="showDetails(expense)">{{ expense.description }}</button><p v-if="expense.reference" class="mt-0.5 truncate text-xs text-slate-500">{{ expense.reference }}</p></td><td class="px-5 py-4">{{ expense.category?.name ?? 'Uncategorized' }}</td><td class="px-5 py-4"><StatusBadge :status="expense.payment_status" /></td><td class="px-5 py-4">{{ expense.payment_method ?? '—' }}</td><td class="px-5 py-4">{{ expense.created_by?.name ?? '—' }}</td><td class="whitespace-nowrap px-5 py-4 text-right font-semibold tabular-nums">{{ formatMoney(expense.amount) }}</td><td class="px-5 py-4"><div class="flex justify-end gap-1"><button class="rounded p-2 text-slate-500 hover:bg-slate-100" aria-label="View expense" @click="showDetails(expense)"><Eye class="size-4" /></button><RouterLink v-if="canEditExpense(expense)" :to="{ name: 'expense-edit', params: { id: expense.id } }" class="rounded p-2 text-slate-500 hover:bg-slate-100" aria-label="Edit expense"><Pencil class="size-4" /></RouterLink><button v-if="auth.isAdmin" class="rounded p-2 text-red-600 hover:bg-red-50" aria-label="Delete expense" @click="confirmDelete(expense)"><Trash2 class="size-4" /></button></div></td></tr></tbody></table></div>
                <div class="divide-y md:hidden"><article v-for="expense in expenses" :key="expense.id" class="p-4"><div class="flex items-start justify-between gap-3"><button class="min-w-0 text-left" @click="showDetails(expense)"><p class="truncate font-medium">{{ expense.description }}</p><p class="mt-1 text-xs text-slate-500">{{ formatDate(expense.expense_date) }} · {{ expense.category?.name ?? 'Uncategorized' }}</p></button><p class="whitespace-nowrap font-semibold">{{ formatMoney(expense.amount) }}</p></div><div class="mt-3 flex items-center justify-between"><StatusBadge :status="expense.payment_status" /><div class="flex gap-2"><RouterLink v-if="canEditExpense(expense)" :to="{ name: 'expense-edit', params: { id: expense.id } }" class="rounded p-2 text-slate-500" aria-label="Edit expense"><Pencil class="size-4" /></RouterLink><button v-if="auth.isAdmin" class="rounded p-2 text-red-600" aria-label="Delete expense" @click="confirmDelete(expense)"><Trash2 class="size-4" /></button></div></div></article></div>
