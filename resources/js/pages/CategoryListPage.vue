...
const toast = useToast(); const categories = ref<Category[]>([]); const loading = ref(true); const saving = ref(false); const error = ref('');
...
async function remove(): Promise<void> {
    ...
    try { await deleteCategory(selected.value.id); deleteOpen.value = false; toast.show('Category deleted'); await load(); }
    catch (caught) { toast.show('Unable to delete category', 'error', apiErrorMessage(caught)); }
    finally { saving.value = false; }
}
function confirmDelete(category: Category): void { selected.value = category; deleteOpen.value = true; }
onMounted(load);
</script>
<template>
    <AppLayout><div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-sm font-medium text-brand-700">Administration</p><h1 class="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">Categories</h1><p class="mt-1 text-sm text-slate-500">Organize expenses into consistent groups.</p></div><BaseButton @click="startCreate"><Plus class="size-4" />Add category</BaseButton></div>
        ...
        <BaseDialog v-model:open="deleteOpen" title="Delete category" description="Existing expenses will be preserved and become uncategorized."><p class="mt-5 text-sm text-slate-600">Delete <strong>{{ selected?.name }}</strong>?</p><div class="mt-6 flex justify-end gap-3"><BaseButton variant="secondary" @click="deleteOpen = false">Cancel</BaseButton><BaseButton variant="danger" :loading="saving" @click="remove">Delete category</BaseButton></div></BaseDialog>
    </AppLayout>
</template>
