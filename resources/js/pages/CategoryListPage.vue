...
<template>
    <AppLayout><div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-sm font-medium text-brand-700">Administration</p><h1 class="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">Categories</h1><p class="mt-1 text-sm text-slate-500">Organize expenses into consistent groups.</p></div><BaseButton @click="startCreate"><Plus class="size-4" />Add category</BaseButton></div>
        ...
        <section v-else class="mt-6 overflow-hidden rounded-xl border bg-white"><EmptyState v-if="!categories.length" title="No categories yet" description="Create the first category to classify office expenses."><BaseButton @click="startCreate">Add category</BaseButton></EmptyState><div v-else class="divide-y"><article v-for="category in categories" :key="category.id" class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div class="flex min-w-0 items-start gap-3"><span class="rounded-lg bg-brand-50 p-2 text-brand-700"><Tags class="size-5" /></span><div><div class="flex items-center gap-2"><h2 class="font-semibold">{{ category.name }}</h2><span class="rounded-full px-2 py-0.5 text-xs font-medium" :class="category.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'">{{ category.is_active ? 'Active' : 'Inactive' }}</span></div><p class="mt-1 text-sm text-slate-500">{{ category.description || 'No description' }}</p></div></div><div class="flex gap-2 self-end sm:self-auto"><button class="rounded-lg border p-2 text-slate-600 hover:bg-slate-50" :aria-label="`Edit ${category.name}`" @click="startEdit(category)"><Pencil class="size-4" /></button><button v-if="category.is_active" class="rounded-lg border p-2 text-red-600 hover:bg-red-50" :aria-label="`Deactivate ${category.name}`" @click="confirmDeactivate(category)"><Trash2 class="size-4" /></button></div></article></div></section>
