...
const error = ref('');
const filters = reactive({ search: '', category_id: '', payment_status: '', payment_method: '', sort: 'expense_date', direction: 'desc' });
...
function clearFilters(): void { Object.assign(filters, { search: '', category_id: '', payment_status: '', payment_method: '', sort: 'expense_date', direction: 'desc' }); load(); }
...
<template>
    <AppLayout>
        ...
        <section class="card mt-6 flex flex-wrap items-end gap-3"><label><span class="label">Month</span><input v-model="period" class="field" type="month" /></label><BaseButton variant="secondary" @click="exportFile('xlsx')">Export Excel</BaseButton><BaseButton variant="secondary" @click="exportFile('pdf')">Export PDF</BaseButton></section>
        <section class="card mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-8"><input v-model="filters.search" class="field mt-0 xl:col-span-2" placeholder="Search descriptions" aria-label="Search" /><select v-model="filters.category_id" class="field mt-0" aria-label="Category"><option value="">All categories</option><option v-for="category in categories" :key="category.id" :value="category.id">{{ category.name }}</option></select><select v-model="filters.payment_status" class="field mt-0" aria-label="Status"><option value="">All statuses</option><option value="paid">Paid</option><option value="unpaid">Unpaid</option><option value="pending">Pending</option><option value="unspecified">Unspecified</option></select><input v-model="filters.payment_method" class="field mt-0" placeholder="Payment method" aria-label="Payment method" /><select v-model="filters.sort" class="field mt-0" aria-label="Sort"><option value="expense_date">Date</option><option value="amount">Amount</option><option value="description">Description</option></select><select v-model="filters.direction" class="field mt-0" aria-label="Direction"><option value="desc">Descending</option><option value="asc">Ascending</option></select><div class="flex gap-2"><BaseButton @click="load()">Apply</BaseButton><BaseButton variant="secondary" @click="clearFilters">Clear</BaseButton></div></section>
