...
const pageNumbers = computed(() => Array.from({ length: report.value?.meta.last_page ?? 1 }, (_, index) => index + 1)
    .filter((page) => Math.abs(page - (report.value?.meta.current_page ?? 1)) <= 2));
const showingFrom = computed(() => !report.value || report.value.meta.total === 0 ? 0 : (report.value.meta.current_page - 1) * report.value.meta.per_page + 1);
const showingTo = computed(() => !report.value ? 0 : Math.min(report.value.meta.current_page * report.value.meta.per_page, report.value.meta.total));
...
<template>
    <AppLayout>
        ...
        <template v-else-if="report"><div class="mt-6"><SummaryCards :summary="report" /></div><section class="card mt-6"><h2 class="font-semibold">Category breakdown</h2><BarChart class="mt-5" :items="report.categories.map(c => ({ label: c.name, total: c.total }))" /><p v-if="!report.categories.length" class="mt-4 text-sm text-slate-500">No expenses in this period.</p></section>
            <section class="mt-6 overflow-hidden rounded-xl border bg-white"><div class="overflow-x-auto"><table class="w-full min-w-[48rem] text-left text-sm"><thead class="bg-slate-50"><tr><th class="p-3">Date</th><th class="p-3">Description</th><th class="p-3">Category</th><th class="p-3">Method</th><th class="p-3">Created by</th><th class="p-3 text-right">Amount</th></tr></thead><tbody class="divide-y"><tr v-for="expense in report.expenses" :key="expense.id"><td class="p-3">{{ formatDate(expense.expense_date) }}</td><td class="p-3 font-medium">{{ expense.description }}</td><td class="p-3">{{ expense.category?.name ?? 'Uncategorized' }}</td><td class="p-3">{{ expense.payment_method ?? '—' }}</td><td class="p-3">{{ expense.created_by?.name ?? '—' }}</td><td class="p-3 text-right font-semibold">{{ formatMoney(expense.amount) }}</td></tr></tbody></table></div><p v-if="!report.expenses.length" class="p-8 text-center text-sm text-slate-500">No transactions match these filters.</p><footer class="flex flex-col gap-3 border-t bg-slate-50 px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between"><span>Showing {{ showingFrom }}–{{ showingTo }} of {{ report.meta.total }}</span><div class="flex flex-wrap gap-2"><BaseButton variant="secondary" :disabled="report.meta.current_page <= 1" @click="load(report.meta.current_page - 1)">Previous</BaseButton><BaseButton v-for="page in pageNumbers" :key="page" :variant="page === report.meta.current_page ? 'primary' : 'secondary'" @click="load(page)">{{ page }}</BaseButton><BaseButton variant="secondary" :disabled="report.meta.current_page >= report.meta.last_page" @click="load(report.meta.current_page + 1)">Next</BaseButton></div></footer></section>
        </template>
    </AppLayout>
</template>
